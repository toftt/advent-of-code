...
export interface OldWithHashValues extends Object {
  hashValues(): Iterable<Hashable>;
}

type Hashable = number | string | boolean | OldWithHashValues | Hashable[];
export class OldHasher {
...
export function oldMakeHash(...values: Hashable[]) {
  const hasher = new OldHasher();
...
export class OldHashSet<T extends Hashable & Eq> {
  private storage = new Map<number, T[]>();
  private _size = 0;

  public add(toInsert: T) {
    const hash = oldMakeHash(toInsert);
